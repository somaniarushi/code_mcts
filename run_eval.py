...
from run_mcts import append_to_jsonl, evaluate_completion
...
def run_eval(outpath: str, n_samples: int) -> None:
    ...
    for task_id, problem in tqdm(problems.items()):
        for i in range(n_samples):
            ...
            assert isinstance(completion, str), f"Expected completion to be a string, but got {type(completion)} {completion=}"

            reward = evaluate_completion(completion, problem)

            append_to_jsonl(output_jsonl_path=filepath, task_id=task_id, completion=completion, reward=reward, rollout_counts_for_solution=-1)
            if reward == 1:
